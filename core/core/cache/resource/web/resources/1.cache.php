<?php  return array (
  ...
  array (
    ...
    'pagetitle' => 'Заказать верстку и программирование сайта. Блочная и адаптивная html + css верстка сайта. Программирование на MODX Revolution',
    'longtitle' => 'Блочная и адаптивная html + css верстка сайта',
    'description' => 'Заказ профессиональной верстки сайта и веб-программирования на MODX Revolution под заказ. Выполняем адаптивную и блочную верстку для сайта.',
    'alias' => 'index',
    'alias_visible' => 1,
    'link_attributes' => '',
    'published' => 1,
    ...
    'isfolder' => 0,
    'introtext' => '',
    'content' => '<p>Результат нашей работы чистый, качественный HTML/CSS код, который передаст всю уникальность и красоту макета дизайна в интернет.</p>

<p>Мы верстаем сайты всех уровней сложности – от самых простых вариантов для блогов и персональных визиток до адаптивной верстки под мобильные устройства. Более 50 сверстанных сайтов разной направленности за 8 месяцев.</p>',
    'richtext' => 0,
    'template' => 2,
    'menuindex' => 0,
    'searchable' => 1,
...
{include \'footer\'}',
  ...
  'elementCache' => 
  array (
    '[[pdoMenu?showLog=``&fastMode=``&level=`2`&parents=``&displayStart=``&resources=``&templates=``&context=``&cache=``&cacheTime=`3600`&cacheAnonymous=``&plPrefix=`wf.`&showHidden=``&showUnpublished=``&showDeleted=``&previewUnpublished=``&hideSubMenus=``&useWeblinkUrl=`1`&sortdir=`ASC`&sortby=`menuindex`&limit=`0`&offset=`0`&rowIdPrefix=``&firstClass=``&lastClass=``&hereClass=`active`&parentClass=`menu-parent`&rowClass=``&outerClass=`menu`&innerClass=``&levelClass=``&selfClass=``&webLinkClass=``&tplOuter=`tplOuter`&tpl=`tpl`&tplParentRow=``&tplParentRowHere=``&tplHere=`tplHere`&tplInner=`tplInner`&tplInnerRow=``&tplInnerHere=``&tplParentRowActive=``&tplCategoryFolder=``&tplStart=`@INLINE <h2[[+classes]]>[[+menutitle]]</h2>[[+wrapper]]`&checkPermissions=``&hereId=``&where=``&select=``&scheme=``&toPlaceholder=``&countChildren=``&startId=`0`&class_inner=`wer`]]' => '<ul class="menu"><li class="active">
<span>Главная</span>
...
<a href="contacts.html">Контакты</a>
    ...
    '[[pdoResources?tpl=`tpl_full_content`&returnIds=``&showLog=``&fastMode=``&sortby=`publishedon`&sortbyTV=``&sortbyTVType=``&sortdir=`DESC`&sortdirTV=`ASC`&limit=`10`&offset=`0`&depth=`10`&outputSeparator=`
`&toPlaceholder=``&parents=`0`&includeContent=`1`&includeTVs=`img`&prepareTVs=`1`&processTVs=``&tvPrefix=`tv.`&tvFilters=``&tvFiltersAndDelimiter=`,`&tvFiltersOrDelimiter=`||`&where=``&showUnpublished=``&showDeleted=``&showHidden=`1`&hideContainers=``&context=``&idx=``&first=``&last=``&tplFirst=``&tplLast=``&tplOdd=``&tplWrapper=``&wrapIfEmpty=``&totalVar=`total`&resources=`6`&tplCondition=``&tplOperator=`==`&conditionalTpls=``&select=``&toSeparatePlaceholders=``&loadModels=``&scheme=``&useWeblinkUrl=``]]' => '<p>Мы верстаем сайты всех уровней сложности – от самых простых вариантов для блогов и персональных визиток до адаптивной верстки, которую  так любит гугл и все мобильные устройства. Более 50 сверстанных сайтов разной направленности  за 2 года. </p>
...
',
    '[[pdoResources?tpl=`tpl_full_content`&returnIds=``&showLog=``&fastMode=``&sortby=`publishedon`&sortbyTV=``&sortbyTVType=``&sortdir=`DESC`&sortdirTV=`ASC`&limit=`10`&offset=`0`&depth=`10`&outputSeparator=`
`&toPlaceholder=``&parents=`0`&includeContent=`1`&includeTVs=`img`&prepareTVs=`1`&processTVs=``&tvPrefix=`tv.`&tvFilters=``&tvFiltersAndDelimiter=`,`&tvFiltersOrDelimiter=`||`&where=``&showUnpublished=``&showDeleted=``&showHidden=`1`&hideContainers=``&context=``&idx=``&first=``&last=``&tplFirst=``&tplLast=``&tplOdd=``&tplWrapper=``&wrapIfEmpty=``&totalVar=`total`&resources=`8`&tplCondition=``&tplOperator=`==`&conditionalTpls=``&select=``&toSeparatePlaceholders=``&loadModels=``&scheme=``&useWeblinkUrl=``]]' => '<div class="row">
    <div class="col-sm-6 col-xs-12">
        <div class="black-block">
            <span class="fa fa-modx"></span>
            <h4>Программирование под CMS MODX Revolution</h4>
            <div>от 3500 рублей</div>
            <p>Интеграция HTML верстки в систему управления сайтом MODX Revo.</p>
            <br>
        </div>
    </div>
    <div class="col-sm-6 col-xs-12">
        <div class="black-block">
            ...
            <h4>Программирование под CMS WordPress</h4>
            <div>от 5 500 рублей</div>
            <p>Интеграция HTML верстки в систему управления сайтом на WordPress.</p>
        </div>
    </div>
</div>

<p>Напишите, что необходимо выполнить, сроки и прикрепите картинку или верстку, чтобы мы видели какой объем работы предстоит выполнить</p>
